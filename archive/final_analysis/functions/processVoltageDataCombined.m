function results_table = processVoltageDataCombined(voltages, dataFolder, prefix)
    ...
    % Processes voltage data files for forced oscillations and compiles results.
    %
    % Inputs:
    %   voltages   - Array of voltage values (numeric)
    %   dataFolder - Folder where the data files are located
    %   prefix     - Optional prefix for filenames (e.g., 'NoPlate')
    %
    % Outputs:
    %   results_table - Table containing processed results
    ...
    for i = 1:n_files
        ...
        if isfile(filepath)
            try
                ...
                angle_amplitude = (max(angle) - min(angle)) / 2;
