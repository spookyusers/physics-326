function [omega0_est, gamma_est] = estimateParameters(frequencies, amplitudes)
    % estimateParameters
    % Estimates the natural frequency (omega0) and damping coefficient (gamma) from experimental data.
    %
    % Inputs:
    ...
    %
    % Outputs:
    %   omega0_est - Estimated natural frequency (rad/s)
    %   gamma_est  - Estimated damping coefficient (1/s)
    ...
    amplitudeModel = @(params, omega) params(1) ./ sqrt((params(2)^2 - omega.^2).^2 + (params(3) .* omega).^2);

    % Initial guesses for [F0_over_m, omega0, gamma]
    [~, max_idx] = max(amplitudes);
    initial_omega0 = frequencies(max_idx); % Guess omega0 where amplitude is maximum
    initial_gamma = 0.1;                   % Initial guess for gamma
    initial_F0_over_m = amplitudes(max_idx) * initial_gamma * initial_omega0; % Based on amplitude at resonance
